...
花了三天时间折腾，终于能够用Siri打开空调，在此记录一下过程。

## 背景
炎炎夏夜，当我风尘仆仆回到家时，只觉得我是从一个大蒸笼回到了一个小蒸笼里，然后打开空调，等待房间温度慢慢下降，我这才觉得我是活着的。

几天前，当我依旧满身汗水回到家，打开空调时，我有了一个想法：我希望在我下班的时候，就能控制空调自动开启，当我回家时，卧室就已经是舒适的温度了。

我在魔都租房住。因为是租房，所以不可以对房间有破坏性的改造，也不可能换一个空调，所以，我需要一个低成本的(因为穷)，小改动量的，同时开发时间较短的(不然等东西做出来夏天就过去了)方案。

## 调研

回忆起大学的暑期实习时，有小伙伴利用MSP430配合红外发射模块控制实验室的空调，于是我也准备通过单片机等设备，控制红外模块，打到控制空调的目的。MSP430板子对开发不友好，我决定用树莓派这种高大上的小玩意。

目的确认后，我对树莓派以及红外模块等相关软硬件做一些调研。
...
* 树莓派
	...
	Raspberry Pi,只有信用卡大小的微型电脑，其系统基于Linux。麻雀虽小，五脏俱全，CPU、GPU、内存、USB、GPIO、Camera、HDMI等功能都具备。树莓派在物联网以及智能家居中的应用非常多，相关的开发也十分方便。
	
	目前最新的版本是"树莓派3代B型"，自带Wifi和蓝牙, 某宝上搜索很容易买到，价格在两三百的范围内。我买的套餐价是306RMB，套餐里有电源线、壳子、散热片、小风扇、SD卡、读卡器、和一个收纳盒。其中，SD卡类比于电脑的硬盘，刷系统时要刷到卡里。电源线不必说，供电要用。这两个是非常必要的，不然树莓派工作不起来。
...
* [lirc](http://www.lirc.org/)
	...
	若要发出的信号空调能识别，必须要知道空调的红外码率以及码表。红外协议中比较有名的是[NEC协议](http://www.cnblogs.com/yulongchen/archive/2013/04/12/3017409.html)，但是因为空调厂商很多，没有一个规范的协议，所以不同的空调，不同的厂商甚至不同的型号，码率不一样。同时因为空调的红外信号中，需要包括温度、风力、模式、定时等相关信息，所以信号比较复杂，需要知道码表才能发出相应的信息。但是码表在网上基本是查不到的，这个都是不开源的。于是需要利用lirc以及红外接收模块，通过录制空调遥控器的信号，来实现简单的空调控制。
...
* [home-assistant](https://home-assistant.io/)
	...
	home-assistant有app端和web端的控制平台。它可以类比于HomeKit，它从人的操作中接收指令，然后运行相关指令，达到控制智能设备的目的。
...
买回来的崭新树莓派以及SD卡，肯定是没有系统的。我们需要自己刷系统。Windows可以参考 [这个文档](HAP-NodeJS)。我的系统是Mac OSX，所以以下步骤为mac下的操作，参考的文档是：[Mac OSX下给树莓派安装Raspbian系统](http://shumeipai.nxez.com/2014/05/18/raspberry-pi-under-mac-osx-to-install-raspbian-system.html?variant=zh-cn)，以下的命令行实例都来自这个文档，因为我已经装过一次系统了→_→，不太方便截图。
...
2. 软件安装
	...
	5. 	录制红外信号
		...
		采用Row方式录制，具体实现就是完完全全记录下红外信号的高低电平以及对应的时间。
		...
		输入命令后，会有说明文字出现，这个时候按以下回车，会出现第二段介绍，再按回车，开始录制了。录制过程就是你不停的按空调遥控器的按钮。理论上每次按空调遥控器后，控制台会多一个点，当出现足够的点后(大约需要40个点)，输入你希望的KEY NAME，然后按对应的空调遥控器的键。此时，模板文件里就会出现这个KEY NAME。输入KEY NAME 以及采集遥控器信号的过程可以循环多次，目的是方便用户一次采集多个KEY NAME。最后多按几次回车结束过程。
...
### HomeKit控制
	...
	这一部分是调研时间最久的，因为花了比较久的时间才分清楚Homebridge相关软件之间的关系。针对我现在的情况：1. 没有使用智能设备 2. 没有现成的插件可用 3. js不熟，写插件的话，要看资料的多，花费的时间长。于是，我选择HAP-NodeJs。
...
# service avahi-daemon start
		...
		一直循环这个过程，直到不报错。这个时候运行Core.js就会启动HAP-NodeJS的服务端，这个时候你的iOS-家庭就能搜到设备了。
...
在调研和实践的过程中，发现我做的事情其实属于智能家居的领域了。

红外遥控的智能化有一个更好的解决方案：BroadLink万能遥控 + Homebridge。

BroadLink的万能遥控是一个类似于机顶盒的东西，内置了大量设备的红外码库，对于无码库的部分机型，你也可以通过它的程序来进行录制操作(和我用红外接收模块录信号一样的原理)。HomeBrige有官方提供的BroadLink插件，所以使用BroadLink + Homebridge是一个更好的方案。
